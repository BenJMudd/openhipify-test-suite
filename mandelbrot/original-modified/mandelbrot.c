...
int main() {
  ...
  */
  dreals = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * nreals,
                          NULL, NULL);
  dimags = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * nimags,
                          NULL, NULL);
  dans = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * nreals, NULL,
                        NULL);

  // Write our data set into the input array in device memory
  err = clEnqueueWriteBuffer(queue, dreals, CL_TRUE, 0, sizeof(float) * nreals,
                             reals, 0, NULL, NULL);
  err |= clEnqueueWriteBuffer(queue, dimags, CL_TRUE, 0, sizeof(float) * nimags,
                              imags, 0, NULL, NULL);
